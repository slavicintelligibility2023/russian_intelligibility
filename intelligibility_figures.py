...
def show_langs(test: str, user_data, axes):
    ...
    sns.swarmplot(x='parallel_lang', y="mean_mark", data=data2, order=order, 
                  size=3, palette="Set2", hue='parallel_lang', 
    ...
    for i in range(1, 9):
        try:
            clr = axes.collections[i].get_facecolor()[0]
            clr_no = 0
            for c_pos, ccc in enumerate(sns.palettes.get_colormap('Set2').colors):
                if clr[0] == ccc[0] and clr[1] == ccc[1] and clr[2] == ccc[2]:
                    clr_no = c_pos
                    break
            axes.text(i+0.1, data3_2.loc[order[i], 'mean_mark']-0.005, 
                      f"+{data3_2.loc[order[i], 'mean_mark'] - data3_2.loc[order[0], 'mean_mark']:5.2}", 
                      color=sns.palettes.get_colormap('Dark2').colors[clr_no], fontsize=12)
        except:
            pass
...
def show_age(field: str, test: str, user_data, axes):
    if test == 'All':
        ...
        data2 = user_data[[field, 'mean_mark']][user_data.test_id == int(test)]
        data3 = user_data[[field, 'mean_mark', 'parallel_lang']][user_data.test_id == int(test)]
    ...
    data3['no parallel text'] = data3['parallel_lang'].str.match('No Parallel Text')
    ...
    if field == 'age':
        if test == 'All':
            ...
            datal = user_data[[field, 'mean_mark', 'parallel_lang']] \
                             [(user_data.test_id == int(test)) & (user_data.speciality=='yes')]
            sns.pointplot(x=field, y="mean_mark", data=datal, color="#00FF00", 
                          markers="x", order=order, err_kws={'linewidth': 1.5}, capsize=0.1, 
                          # join=False, 
                          legend=None, ax=axes) 
            datan = user_data[[field, 'mean_mark', 'parallel_lang']] \
                             [(user_data.test_id == int(test)) & (user_data.speciality=='no')]
            sns.pointplot(x=field, y="mean_mark", data=datan, color="#0000FF", 
                          markers="x", order=order, err_kws={'linewidth': 1.5}, capsize=0.1, 
                          # join=False, 
                          ax=axes) 
    ...
    # _ = moveCollection(axes, -2, 18/72., "left")
...
def processIntelligibility(data, texts):
    ...
    for c_lang, chang in changes.items():
        for ch in chang:
            words1.loc[ch[0], c_lang], words1.loc[ch[1], c_lang] = words1.loc[ch[1], c_lang], words1.loc[ch[0], c_lang]
            rf.loc[('1',ch[0]), c_lang], rf.loc[('1',ch[1]), c_lang] = rf.loc[('1',ch[1]), c_lang], rf.loc[('1',ch[0]), c_lang]
